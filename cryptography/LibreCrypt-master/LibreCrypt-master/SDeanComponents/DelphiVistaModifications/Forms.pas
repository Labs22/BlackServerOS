...
uses
  ActiveX, Math, Printers, Consts, RTLConsts, CommCtrl, FlatSB, StdActns, Themes;
  ...
  begin
    ...
    begin
      ...
      if (FInternalPopupParent = nil) and (FInternalPopupParentWnd = 0) then
        case LPopupMode of
// VISTA FIX -          pmNone: WndParent := Application.Handle;
          pmNone, pmAuto:
            begin
...
begin
  if (csDesigning in ComponentState) and (Parent <> nil) then
  begin
    DesignInfo := (DesignInfo and $FFFF0000) or (Value and $FFFF);
    if not (csLoading in ComponentState) and (Position <> poDefaultSizeOnly) then
      Position := poDesigned;
  end else
    inherited Top := Value;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        EnumWindows(@GetPopupOwnerWindows, 0);
        if FPopupOwners.Count > 0 then
...
begin
  ...
  begin
    ...
    if csDesigning in ActiveForm.ComponentState then
      CallHelp := False;
