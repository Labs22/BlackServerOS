...
var
  ParentWnd: HWND;
begin
// Vista fix -   if Application.ModalPopupMode <> pmNone then
// Vista fix -   begin
...
// Vista fix -   end
// Vista fix -   else
// Vista fix -     ParentWnd := Application.Handle;
...
begin
  ...
  begin
    ...
    //Flags := OFN_ENABLEHOOK;  // Vista fix
    Flags := 0;  // Vista fix
    FlagsEx := 0;
...
// Vista fix -     if Template <> nil then
// Vista fix -     begin
// Vista fix -       Flags := Flags or OFN_ENABLETEMPLATE;
// Vista fix -       lpTemplateName := Template;
// Vista fix -       if FTemplateModule <> 0 then
// Vista fix -         hInstance := FTemplateModule;
// Vista fix -     end;
// Vista fix -     if Application.ModalPopupMode <> pmNone then
// Vista fix -     begin
// Vista fix -       FRedirector := TRedirectorWindow.Create(nil);
// Vista fix -       with TRedirectorWindow(FRedirector) do
// Vista fix -       begin
// Vista fix -         FCommonDialog := Self;
// Vista fix -         FFormHandle := ParentWnd;
// Vista fix -       end;
// Vista fix -       hWndOwner := FRedirector.Handle;
// Vista fix -     end
// Vista fix -     else
// Vista fix -       hWndOwner := Application.Handle;
    hWndOwner := ParentWnd;  // Vista fix
