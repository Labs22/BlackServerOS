...
begin
  ...
  RegisterComponents('OTFE', [TOTFEDriveCrypt]);
end;

(******************************************************************************
                         TOTFEDriveCrypt DEFINITIONS
 ******************************************************************************)

{ --- Standard component and connection routines --- }

constructor TOTFEDriveCrypt.Create(AOwner : TComponent);
var
  i : integer;
  OSversion: DWORD;
begin
...
destructor TOTFEDriveCrypt.Destroy;
begin
  if FActive then
...
function TOTFEDriveCrypt.Connect(): boolean;
var
  pchDriver : PChar;
...
begin
  ...
  if not(Active) then
    ...
    if not(RunningOnNT) then
      begin
      hScramDiskVxD := CreateFile('\\.\dcr', 0, 0, nil, 0, FILE_FLAG_DELETE_ON_CLOSE, 0);
      ...
      begin
      hScramDiskVxD := CreateFile('\\.\DCR', 0, (FILE_SHARE_READ or FILE_SHARE_WRITE), nil, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
      if hScramDiskVxD=INVALID_HANDLE_VALUE then
        begin
...
function TOTFEDriveCrypt.Disconnect(): boolean;
begin
  FLastErrCode:= OTFE_ERR_SUCCESS;
  if Active then
    begin
...
function TOTFEDriveCrypt.GetSlotInfo(SlotNo : Integer) : TSlotInfo;
var
  dwBytesReturned: DWORD;
...
procedure TOTFEDriveCrypt.UpdateSlotInfo;
var
  dwBytesReturned : DWORD;
  c: char;
  dbit: integer;
...
function TOTFEDriveCrypt.PerformMountContainer(Filename: string; clearPasswordsAfterwards: boolean): boolean;
var
  fs : TMountFileStruct;
  mf : ^TMountFileStruct;
  dwBytesReturned : DWORD;
...
procedure TOTFEDriveCrypt.MountAll;
begin
  MountPartitions();
...
// calling it to do
procedure TOTFEDriveCrypt.MountPartitions;
var
  dwBytesReturned : DWORD;
begin
  CheckActive();
...
function  TOTFEDriveCrypt.Dismount(driveLetter: char; emergency: boolean = FALSE): boolean;
//procedure TOTFEDriveCrypt.Dismount(Drive : string; Brutal : Boolean);
var
  SlotNo : Integer;
  ExamineSlot : Integer;
  slotInfo: TSlotInfo;
...
procedure TOTFEDriveCrypt.Dismount(SlotNo : Integer; Brutal : Boolean);
begin
  CheckActive();
...
procedure TOTFEDriveCrypt.BrutalDismountSlot(SlotNo : Integer);
var
  dwBytesReturned : DWORD;
  CloseMode : Integer;
...
function TOTFEDriveCrypt.FindWavData(fs : TFileStream; var wavInfo: TWavInfo) : Integer;
const
  STR_RIFF = 'RIFF';
  STR_WAVEFMT = 'WAVEfmt ';
  STR_DATA = 'data';
...
function TOTFEDriveCrypt.CheckWav(Filename: string; var wavInfo: TWavInfo) : Integer;
var
  valid : Integer;
...
function TOTFEDriveCrypt.SetPasswordsRedScreen(ScreenTitle : string) : Boolean;
var
  dwBytesReturned : DWORD;
  messagestr : PChar;
  accepted : LongInt;
...
procedure TOTFEDriveCrypt.SetPasswords(Password1, Password2, Password3, Password4 : string);
begin
  SetPasswords(FALSE, Password1, Password2, Password3, Password4);
end;


procedure TOTFEDriveCrypt.SetPasswords(skipActiveCheck: boolean; Password1, Password2, Password3, Password4 : string);
var
...
procedure TOTFEDriveCrypt.ClearAllPasswords();
begin
...
// Clear down the cached passwords
procedure TOTFEDriveCrypt.ClearAllPasswords(skipActiveCheck: boolean);
var
...
procedure TOTFEDriveCrypt.GetPasswordBufferInt;
var
  dwBytesReturned : DWORD;
begin
  dwBytesReturned := 0; // Required to get rid of compiler warning
...
procedure TOTFEDriveCrypt.PassPasswords;
var
  dwBytesReturned : DWORD;
...
procedure TOTFEDriveCrypt.RefreshApplication;
var
  dwBytesReturned : DWORD;
begin
  dwBytesReturned := 0; // Required to get rid of compiler warning
...
function TOTFEDriveCrypt.EjectStop(DriveLetter : string; EjectFunction : Boolean) : Boolean;
var
  hVWin32VxD : THandle;
...
procedure TOTFEDriveCrypt.NTEjectStopNT(DriveLetter: char; EjectFunction: boolean);
var
  volEjectName: string;
  func: integer;
begin
...
procedure TOTFEDriveCrypt.UpdateDeviceDriverTime;
var
  s : TSystemTime;
  dwBytesReturned : DWORD;
begin
...
procedure TOTFEDriveCrypt.SetActive(AValue : Boolean);
var
  allOK: boolean;
begin
  allOK := FALSE;
...
function TOTFEDriveCrypt.IOCtlLock(DriveNo : Integer; Permissions : Integer; FunctionNo : Integer) : Integer;
var
  hDevice : THandle;
...
// i.e. A: = 1, B: = 2, etc...
function TOTFEDriveCrypt.LockLogDrive(DriveNo, Mode: Integer) : Integer;
var
  a : Integer;
begin
  if (mode > 0) then
...
function TOTFEDriveCrypt.LockDrive(Drive : string; Mode : Integer) : Boolean;
var
  DriveLett : Integer;
  a : Integer;
  UpCaseDrive : string;
...
function TOTFEDriveCrypt.GetWordFromStream(ts: TFileStream): WORD;
var
...
function TOTFEDriveCrypt.GetDWordFromStream(ts: TFileStream): DWORD;
var
  a, b: WORD;
begin
...
function TOTFEDriveCrypt.MountPrompted(volumeFilename: string; readonly: boolean): boolean;
begin
...
function TOTFEDriveCrypt.EnterPasswordsDialog() : boolean;
const
  ANTI_SWAP = 512*1024;
var
...
// readonly
// Returns: TRUE on success, otherwise FALSE
function TOTFEDriveCrypt.MountContainer(filename: string; readonly: boolean; clearPasswordsAfterwards: boolean): boolean;
var
...
function TOTFEDriveCrypt.SetReadonlyAttrib(filename: string; setNotUnset: boolean) : boolean;
var
  fileAttributes : integer;
begin
  fileAttributes := FileGetAttr(filename);
...
function TOTFEDriveCrypt.MaxSlots() : integer;
begin
  Result := MAX_SCRAMDISK_SLOTS + 1;
end;

function TOTFEDriveCrypt.NumberMounted() : integer;
begin
  Result := CountDrivesMounted();
end;

function  TOTFEDriveCrypt.Dismount(volumeFilename: string; emergency: boolean = FALSE): boolean;
begin
  Result := Dismount(GetDriveForVolFile(volumeFilename), emergency);

end;

function  TOTFEDriveCrypt.IsEncryptedVolFile(volumeFilename: string): boolean;
begin
...
function  TOTFEDriveCrypt.DrivesMounted(): string;
var
  slotInfo : TSlotInfo;
  i : integer;
  mtdDrvs: string;
...
// not mounted
function TOTFEDriveCrypt.GetDriveLetterForVolFile(volumeFilename: string): string;
begin
  Result := GetDriveForVolFile(volumeFilename);

end;

function  TOTFEDriveCrypt.GetDriveForVolFile(volumeFilename: string): char;
var
  i: integer;
...
function  TOTFEDriveCrypt.GetVolFileForDrive(driveLetter: char): string;
var
  i: integer;
  slotInfo: TSlotInfo;
begin
...
function TOTFEDriveCrypt.Title(): string;
begin
  Result := 'DriveCrypt';
end;


function TOTFEDriveCrypt.Version(): cardinal;
var
  pchDriver : PChar;
  iDriverVersion : ^Integer;
  dwBytesReturned : DWORD;
...
function TOTFEDriveCrypt.GetSlotForDrive(driveLetter: char): integer;
var
  i: integer;
  slotInfo: TSlotInfo;
begin
...
function TOTFEDriveCrypt.GetVolumeInfo(driveLetter: char; var info: TSlotInfo): boolean;
var
  slot: integer;
begin
  Result := FALSE;
...
// Returns TRUE on success, otherwise FALSE
function TOTFEDriveCrypt.MountPrompted(var drive: string) : boolean;
var
...
function TOTFEDriveCrypt.MountPartitionsPrompted(): string;
var
  passwordsEntered : boolean;
  beforeDrvs: string;
  afterDrvs: string;
...
function TOTFEDriveCrypt.DismountPrompted() : boolean;
var
...
begin
  ...
  if NumberMounted()<1 then
    ...
    try
      dismountDlg.ScramDiskComponent := TOTFEScramDisk(self);
...
function TOTFEDriveCrypt.VersionStr(): string;
var
  verNo: cardinal;
  majorVer: integer;
  minorVer: integer;
...
function TOTFEDriveCrypt.GetMainExe(): string;
begin
...
function TOTFEDriveCrypt.NTLoadLocalDriver(): THandle;
var
  hdriver: THandle;
  error: DWORD;
  sysDir: string;
...
function TOTFEDriveCrypt.NTUnloadDeviceDriver(Name: string): boolean;
var
...
function TOTFEDriveCrypt.NTLoadDeviceDriver(Name: string; Path: string; var lphDevice: THandle; var Error: DWORD): boolean;
var
  schSCManager: SC_HANDLE;
  okay: boolean;
begin
...
function TOTFEDriveCrypt.ResetSDRegistry(): boolean;
const
  pridisk: string =' PrimaryDisk';
  sysroot: string = '\SystemRoot\System32\drivers\DCR.SYS';
var
  dwErrorCode: DWORD;
  theHKey: HKEY;
  dwDisposition: DWORD;
...
begin
  ...
  RegDeleteKey(HKEY_LOCAL_MACHINE,'System\CurrentControlSet\Services\DCR\Security');
  RegDeleteKey(HKEY_LOCAL_MACHINE,'System\CurrentControlSet\Services\DCR\Enum');
  RegDeleteKey(HKEY_LOCAL_MACHINE,'System\CurrentControlSet\Services\DCR');

  dwErrorCode := RegCreateKeyEx(HKEY_LOCAL_MACHINE,
                                'System\CurrentControlSet\Services\DCR',
                                0,
                                '',
...
function TOTFEDriveCrypt.StopDriver(SchSCManager: SC_HANDLE; DriverName: string): boolean;
var
...
function TOTFEDriveCrypt.RemoveDriver(SchSCManager: SC_HANDLE; DriverName: string): boolean;
var
  schService: SC_HANDLE;
  ret: boolean;
begin
...
function TOTFEDriveCrypt.InstallDriver(SchSCManager: SC_HANDLE; DriverName: string; ServiceExe: string): boolean;
var
  schService: SC_HANDLE;
begin
...
function TOTFEDriveCrypt.StartDriver(SchSCManager: SC_HANDLE; DriverName: string): boolean;
var
  schService: SC_HANDLE;
  ret: boolean;
...
function TOTFEDriveCrypt.OpenDevice(DriverName: string; var lphDevice: THandle): boolean;
var
...
procedure TOTFEDriveCrypt.DEBUG_REPORTERROR(e: integer);
begin
  case e of
...
function TOTFEDriveCrypt.SDXDeviceIoControl(hDevice: THandle;       // handle to device of interest
                                           dwIoControlCode: DWORD; // control code of operation to perform
                                           lpInBuffer: Pointer;    // pointer to buffer to supply input data
                                           nInBufferSize: DWORD;   // size of input buffer
                                           lpOutBuffer: Pointer;   // pointer to buffer to receive output data
...
procedure TOTFEDriveCrypt.NTMountSDPartitions(crit: PChar);
var
  i: integer;
  mf: TMountFileStruct;
begin
...
procedure TOTFEDriveCrypt.MountNTVol(drive: char);
var
...
function TOTFEDriveCrypt.openread(filename: string): THandle;
var
  h: THandle;
begin
...
function TOTFEDriveCrypt.IsDriveReadonly(driveLetter: char): boolean;
var
  slotNo: integer;
  slotInfo: TSlotInfo;
begin
...
function TOTFEDriveCrypt.DetermineMountType(slotNo: integer): TMountType;
var
...
// DetermineMountType for ScramDisk v3+
function TOTFEDriveCrypt.DetermineMountType(cv: TCryptVol): TMountType;
begin
  Result := mtNotMounted;

  if (Version()<$300) then
    begin
    raise EOTFEException.Create('TOTFEDriveCrypt.DetermineMountType may only be called with a TCryptVol with ScramDisk v3 or later');
    end
  else
    begin
    if (cv.booted=0) then
...
procedure TOTFEDriveCrypt.ResetTimeStamp(filename: string);
var
  hFile: THandle;
...
function TOTFEDriveCrypt.NTCheckStreaming(slotNo: integer): boolean;
var
  slotnum: integer;
  closemode: integer;
  i: integer;
...
function TOTFEDriveCrypt.NTDismountVolume(brutal: integer; drv: integer; slot: integer): boolean;
var
...
function TOTFEDriveCrypt.RecursedContainer(drv: char): boolean;
begin
...
// xxx - put NT dismount stuff in a different file?
procedure TOTFEDriveCrypt.Refresh_MyComp();
var
  sf: IShellFolder;
  p: PItemIDList;
  stt: STRRET;
...
procedure TOTFEDriveCrypt.UniCpy(var output: string; pwidestring: LPWSTR);
var
  s: DWORD;
  i: cardinal;
  wStr: widestring;
...
// xxx - put NT dismount stuff in a different file?
function TOTFEDriveCrypt.NTDoDeviceCloseNT(slotNo: integer; brutal: integer): boolean;
var
...
procedure TOTFEDriveCrypt.NTReadAllPartitionsNT();
var
  devstr: string;
  parstr: string;
  pc: integer;
...
function TOTFEDriveCrypt.NTReadNTPartition(physdrive: integer; var driveLayout: TDRIVE_LAYOUT_INFORMATION): boolean;
var
  Ipartstr: string;
  li: TDRIVE_LAYOUT_INFORMATION;
  Pdevname: string;
...
procedure TOTFEDriveCrypt.DEBUG_GetDriveInfo(physdrive: integer; var driveLayout: TDRIVE_LAYOUT_INFORMATION);
begin
  CheckActive();
...
function TOTFEDriveCrypt.ValidScramDiskPartitions(partitionNames: TStringList): boolean;
begin
  CheckActive();
...
procedure TOTFEDriveCrypt.SetPreferredDrive(drive: char);
begin
  FStartDrive:=(ord(upcase(drive))-ord('A'));

end;

function TOTFEDriveCrypt.GetPreferredDrive(): char;
begin
...
// ScramDisk driver
procedure TOTFEDriveCrypt.SniffPassword(var password1 : string;
                                       var password2 : string;
                                       var password3 : string;
                                       var password4 : string);
var
...
// Enable/disable the ScramDisk driver's ".SVL" file protection
function TOTFEDriveCrypt.SetEnableVolumeDeletion(enable: boolean): boolean;
var
  en: integer;
  dwBytesReturned: DWORD;
begin
