...
#define tk3(j)  t_key[j] = t1 = rotl(t_key[j] + s_box[t1 & 511], 9)
    ...
    for(i = 5; i < 37; i += 2)
    {
        ...
        if(m)
        {
            ...
            j ^= (rotl(b_tab[context->l_key[i] & 3], context->l_key[i - 1]) & m);
        }
...
void MARS(encrypt)(MARS(CONTEXT) *context, const u1byte in_blk[16], u1byte out_blk[16])
{   u4byte  a, b, c, d, l, m, r;

    a = u4byte_in(in_blk    ) + context->l_key[0]; b = u4byte_in(in_blk +  4) + context->l_key[1];
    c = u4byte_in(in_blk + 8) + context->l_key[2]; d = u4byte_in(in_blk + 12) + context->l_key[3];
    ...
    u4byte_out(out_blk,     a - context->l_key[36]); u4byte_out(out_blk +  4, b - context->l_key[37]);
    u4byte_out(out_blk + 8, c - context->l_key[38]); u4byte_out(out_blk + 12, d - context->l_key[39]);
}

void MARS(decrypt)(MARS(CONTEXT) *context, const u1byte in_blk[16], u1byte out_blk[16])
{   u4byte  a, b, c, d, l, m, r;

    d = u4byte_in(in_blk    ) + context->l_key[36]; c = u4byte_in(in_blk +  4) + context->l_key[37];
    b = u4byte_in(in_blk + 8) + context->l_key[38]; a = u4byte_in(in_blk + 12) + context->l_key[39];
    ...
    u4byte_out(out_blk,     d - context->l_key[0]); u4byte_out(out_blk +  4, c - context->l_key[1]);
    u4byte_out(out_blk + 8, b - context->l_key[2]); u4byte_out(out_blk + 12, a - context->l_key[3]);
}
