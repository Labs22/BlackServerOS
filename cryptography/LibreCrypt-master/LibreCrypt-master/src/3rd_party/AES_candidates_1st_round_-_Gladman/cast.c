...
#define f_rnd(x,n)                              \
    f1(x[2],x[3],context->l_key[n],    context->l_key[n + 4]);    \
    f2(x[1],x[2],context->l_key[n + 1],context->l_key[n + 5]);    \
    f3(x[0],x[1],context->l_key[n + 2],context->l_key[n + 6]);    \
    f1(x[3],x[0],context->l_key[n + 3],context->l_key[n + 7])
...
u4byte *set_key(CONTEXT *context, const u4byte in_key[], const u4byte key_len)
{   u4byte  i, j, t, u, cm, cr, lk[8], tm[8], tr[8];
    ...
    for(i = 0; i < 96; i += 8)
    {
        ...
        context->l_key[i + 0] = lk[0]; context->l_key[i + 1] = lk[2];
        context->l_key[i + 2] = lk[4]; context->l_key[i + 3] = lk[6];
        context->l_key[i + 4] = lk[7]; context->l_key[i + 5] = lk[5];
        context->l_key[i + 6] = lk[3]; context->l_key[i + 7] = lk[1];
    }

    return context->l_key;
};

/* encrypt a block of text  */

void encrypt(CONTEXT *context, const u4byte in_blk[4], u4byte out_blk[])
{   u4byte  t, u, blk[4];
...
void decrypt(CONTEXT *context, const u4byte in_blk[4], u4byte out_blk[4])
{   u4byte  t, u, blk[4];
