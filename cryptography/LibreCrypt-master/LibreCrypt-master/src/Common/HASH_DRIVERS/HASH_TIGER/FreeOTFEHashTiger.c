...
)
{
    ...
    FREEOTFE_MEMCPY(
                  ...
                  DRIVER_HASH_TITLE_TIGER,
                  strlen(DRIVER_HASH_TITLE_TIGER)
                 );

    driverInfo->HashDetails[idx].Length = (tiger_desc.hashsize * 8);
    driverInfo->HashDetails[idx].BlockSize = (tiger_desc.blocksize * 8);
    driverInfo->HashDetails[idx].VersionID = DRIVER_HASH_IMPL_VERSION;
    driverInfo->HashDetails[idx].HashGUID = HASH_GUID_TIGER;
...
)
{
    ...
    if (IsEqualGUID(&HASH_GUID_TIGER, HashGUID))
        {
        if (*HashLength < (tiger_desc.hashsize * 8))
            {
            DEBUGOUTHASHIMPL(DEBUGLEV_ERROR, (TEXT("output hash length buffer too small (got: %d; need: %d)\n"),
                *HashLength,
                (tiger_desc.hashsize * 8)
                ));
            ...
            {
            tiger_desc.init(&md);
            tiger_desc.process(&md, Data, (DataLength / 8));
            tiger_desc.done(&md, Hash);
        
            *HashLength = (tiger_desc.hashsize * 8);
            }
