...
#include "FreeOTFECypherSerpent_Gladman.h"
...
)
{
    ...
    FREEOTFE_MEMCPY(
                  ...
                  DRIVER_CIPHER_TITLE_SERPENT_256_XTS,
                  strlen(DRIVER_CIPHER_TITLE_SERPENT_256_XTS)
                 );

    driverInfo->CypherDetails[idx].Mode              = CYPHER_MODE_XTS;
    driverInfo->CypherDetails[idx].KeySizeUnderlying = 256;
    driverInfo->CypherDetails[idx].BlockSize         = SERPENT_BLOCK_SIZE;
    driverInfo->CypherDetails[idx].VersionID         = DRIVER_CYPHER_IMPL_VERSION;
    driverInfo->CypherDetails[idx].CypherGUID        = CIPHER_GUID_SERPENT_256_XTS;
    ...
    FREEOTFE_MEMCPY(
                  driverInfo->CypherDetails[idx].Title,
                  DRIVER_CIPHER_TITLE_SERPENT_192_XTS,
                  strlen(DRIVER_CIPHER_TITLE_SERPENT_192_XTS)
                 );
    ...
    driverInfo->CypherDetails[idx].BlockSize         = SERPENT_BLOCK_SIZE;
    driverInfo->CypherDetails[idx].VersionID         = DRIVER_CYPHER_IMPL_VERSION;
    driverInfo->CypherDetails[idx].CypherGUID        = CIPHER_GUID_SERPENT_192_XTS;
    driverInfo->CypherDetails[idx].KeySizeRequired   = (2 * driverInfo->CypherDetails[idx].KeySizeUnderlying);
    ...
    FREEOTFE_MEMCPY(
                  driverInfo->CypherDetails[idx].Title,
                  DRIVER_CIPHER_TITLE_SERPENT_128_XTS,
                  strlen(DRIVER_CIPHER_TITLE_SERPENT_128_XTS)
                 );
    ...
    driverInfo->CypherDetails[idx].BlockSize         = SERPENT_BLOCK_SIZE;
    driverInfo->CypherDetails[idx].VersionID         = DRIVER_CYPHER_IMPL_VERSION;
    driverInfo->CypherDetails[idx].CypherGUID        = CIPHER_GUID_SERPENT_128_XTS;
    driverInfo->CypherDetails[idx].KeySizeRequired   = (2 * driverInfo->CypherDetails[idx].KeySizeUnderlying);
    ...
    FREEOTFE_MEMCPY(
                  driverInfo->CypherDetails[idx].Title,
                  DRIVER_CIPHER_TITLE_SERPENT_256_CBC,
                  strlen(DRIVER_CIPHER_TITLE_SERPENT_256_CBC)
                 );
    ...
    driverInfo->CypherDetails[idx].BlockSize         = SERPENT_BLOCK_SIZE;
    driverInfo->CypherDetails[idx].VersionID         = DRIVER_CYPHER_IMPL_VERSION;
    driverInfo->CypherDetails[idx].CypherGUID        = CIPHER_GUID_SERPENT_256_CBC;
    ...
    FREEOTFE_MEMCPY(
                  driverInfo->CypherDetails[idx].Title,
                  DRIVER_CIPHER_TITLE_SERPENT_192_CBC,
                  strlen(DRIVER_CIPHER_TITLE_SERPENT_192_CBC)
                 );
    ...
    driverInfo->CypherDetails[idx].BlockSize         = SERPENT_BLOCK_SIZE;
    driverInfo->CypherDetails[idx].VersionID         = DRIVER_CYPHER_IMPL_VERSION;
    driverInfo->CypherDetails[idx].CypherGUID        = CIPHER_GUID_SERPENT_192_CBC;
    driverInfo->CypherDetails[idx].KeySizeRequired   = driverInfo->CypherDetails[idx].KeySizeUnderlying;
    ...
    FREEOTFE_MEMCPY(
                  driverInfo->CypherDetails[idx].Title,
                  DRIVER_CIPHER_TITLE_SERPENT_128_CBC,
                  strlen(DRIVER_CIPHER_TITLE_SERPENT_128_CBC)
                 );
    ...
    driverInfo->CypherDetails[idx].BlockSize         = SERPENT_BLOCK_SIZE;
    driverInfo->CypherDetails[idx].VersionID         = DRIVER_CYPHER_IMPL_VERSION;
    driverInfo->CypherDetails[idx].CypherGUID        = CIPHER_GUID_SERPENT_128_CBC;
    driverInfo->CypherDetails[idx].KeySizeRequired   = driverInfo->CypherDetails[idx].KeySizeUnderlying;
...
)
{
    ...
    char tmpBuffer[(SERPENT_BLOCK_SIZE / 8)];
    int i;
    INTEGER_128 blockID128;
    int errnum;
    SERPENT(CONTEXT) context;

    DEBUGOUTCYPHERIMPL(DEBUGLEV_ENTER, (TEXT("ImpCypherEncryptData\n")));

    if (
          (IsEqualGUID(&CIPHER_GUID_SERPENT_256_XTS, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_SERPENT_192_XTS, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_SERPENT_128_XTS, CypherGUID)) 
         )
        {
        ...
        if ((errnum = xts_encrypt(
          ...
          (IsEqualGUID(&CIPHER_GUID_SERPENT_256_CBC, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_SERPENT_192_CBC, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_SERPENT_128_CBC, CypherGUID)) 
         )
        {
        ...
        //       PlaintextData
        SERPENT(set_key)(&context, Key, KeyLength, enc);
        for (i=0; i < PlaintextLength; i += (SERPENT_BLOCK_SIZE / 8))
            {
            XORBlock(PlaintextData+i, IV, tmpBuffer, (SERPENT_BLOCK_SIZE / 8));
            SERPENT(encrypt)(&context, tmpBuffer, CyphertextData+i);
            IV = CyphertextData+i;
            }
...
)
{
    ...
    if (
        ...
        SERPENT(set_key)(&context, Key, KeyLength, dec);
        for (i=0; i < CyphertextLength; i += (SERPENT_BLOCK_SIZE / 8))
            {
            SERPENT(decrypt)(&context, CyphertextData+i, PlaintextData+i);
            XORBlock(PlaintextData+i, IV, PlaintextData+i, (SERPENT_BLOCK_SIZE / 8));
            IV = CyphertextData+i;
            }
