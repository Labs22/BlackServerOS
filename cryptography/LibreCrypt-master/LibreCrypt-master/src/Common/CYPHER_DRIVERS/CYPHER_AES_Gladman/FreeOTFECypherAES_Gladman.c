...
// This line from aes_aux.c required
...
)
{
    ...
    driverInfo->CypherDetails[idx].BlockSize         = AES_BLOCK_SIZE;
...
)
{
    ...
    if (
          (IsEqualGUID(&CIPHER_GUID_AES_256_XTS, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_AES_192_XTS, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_AES_128_XTS, CypherGUID))
         )
        {
        ...
        if ((errnum = xts_encrypt(
          ...
          (IsEqualGUID(&CIPHER_GUID_AES_192_CBC, CypherGUID)) ||
          (IsEqualGUID(&CIPHER_GUID_AES_128_CBC, CypherGUID))
         )
        {
        ...
        //       PlaintextData
        RIJNDAEL(set_key)(&context, Key, KeyLength, enc);
        for (i=0; i < PlaintextLength; i += (AES_BLOCK_SIZE / 8))
            {
            XORBlock(PlaintextData+i, IV, tmpBuffer, (AES_BLOCK_SIZE / 8));
            RIJNDAEL(encrypt)(&context, tmpBuffer, CyphertextData+i);
            IV = CyphertextData+i;
            }
...
)
{
    ...
    if (
        ...
        RIJNDAEL(set_key)(&context, Key, KeyLength, dec);
        for (i=0; i < CyphertextLength; i += (AES_BLOCK_SIZE / 8))
            {
            RIJNDAEL(decrypt)(&context, CyphertextData+i, PlaintextData+i);
            XORBlock(PlaintextData+i, IV, PlaintextData+i, (AES_BLOCK_SIZE / 8));
            IV = CyphertextData+i;
            }
