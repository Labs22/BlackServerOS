...
procedure TfrmProperties.pbApplyClick(Sender: TObject);
begin
  // Do nothing...
end;

procedure TfrmProperties.pbOKClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmProperties.SetupCtlCheckbox(chkBox: TCheckbox);
begin
  // Nothing - should really set to readonly, since we don't support letting
  // the user change these
end;

procedure TfrmProperties.SetupCtlSeparatorPanel(pnl: TPanel);
begin
  pnl.Caption    := '';
  pnl.Height     := 3;
