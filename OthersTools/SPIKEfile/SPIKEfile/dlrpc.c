...
dlrpc_call (struct dlargs *mydlargs)
{
  ...
  for (i = 0; i < length; i++)
    {
      if (dldemarshall (&args[i], tmpdlargs->dataType, tmpdlargs->data) ==
	  FAILURE)
	{
  ...
  switch (length)
    ...
    case 4:
      vreturn =
	((void *(*)()) (p)) (args[0], args[1], args[2], args[3], args[4]);
      break;
    case 5:
      vreturn =
	((void *(*)()) (p)) (args[0], args[1], args[2], args[3], args[4],
...
*/
int
get_phrase (unsigned char *start, unsigned char **newstring, int state,
	    unsigned char **next)
{
  char *c;
  int result;
  ...
  switch (state)
      ...
      if (*c == '"')
	{
	  ...
	  result =
	    parse_quoted_string (c, (char **) &argumentstring,
				 (char **) &newposition);
	  switch (result)
	    {
	    case SUCCESS:
...
int
s_parse_line (char *line, char *currentvariable, int linenumber,
	      struct dlargs *mydlargs, int state, char *c)
{
...
#endif
      result = get_phrase (c, &newbuffer, state, &nextchar);
  ...
  while (c != c2)
    {
      ...
      s_parse_line (linebuffer, currentvariable, linenumber, mydlargs, state,
		    c);
