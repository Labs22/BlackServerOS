...
s_dce_bind (char *uuid, char *transfer_syntax,
	    ...
	    unsigned int interfaceversionminor, unsigned int syntaxversion);

int s_dce_string (unsigned char *mystring);

int
s_dce_call_header (int callid, int allochint, unsigned short opnum, int first,
		   int last, unsigned char *object);
...
/*assumes bind has already been done*/
int
s_do_dce_call (unsigned short function_number, unsigned char *buffer,
	       unsigned int length, unsigned char *object,
	       //following for UDP only
	       unsigned char *ifid, unsigned int ifid_ver,
	       unsigned char *activity_id);

/*SMB stuff*/
int s_smb_negotiate ();

int s_smb_session_setup ();
...
int
s_smb_dce_call (unsigned short function_number, unsigned char *buffer,
		unsigned int length, unsigned char *fid, unsigned char *uid,
		unsigned char *tid, unsigned char *object);

void
s_smb_writex_header (int first, unsigned char *fid, unsigned char *uid,
		     unsigned char *tid);
