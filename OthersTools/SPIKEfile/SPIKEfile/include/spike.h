...
struct spike *new_spike ();

int spike_free (struct spike *old_spike);

int s_string (char *instring);

int s_blocksize_string (char *instring, int size);
...
*/
int s_push (unsigned char *pushme, unsigned long size);

int setspike (struct spike *newspike);

/*if you need a reserved source port (<1024) set this to true*/
int s_set_needreserved (int needreserved);

int s_block_start (char *blockname);

int s_binary_block_size_word_bigendian (char *blockname);

int s_block_end (char *blockname);

void s_print_buffer ();

void s_printf_buffer ();
...
/*connects and sends*/
int spike_send_tcp (char *host, int port);
...
int s_bigword (unsigned int aword);

/*many protocols have a "size" they want to add a certain constant to - maybe
  as a flag the way RPC does with the first word?*/
int s_binary_block_size_word_bigendian_plussome (char *blockname, long some);

/*none of the "mult" api calls can be done from a .spk script!*/
int s_binary_block_size_byte_mult (char *blockname, float mult);


int s_binary_block_size_word_bigendian_mult (char *blockname, float mult);

int
s_binary_block_size_word_intel_mult_plus (char *blockname, long some,
					  float mult);

int s_binary_block_size_intel_halfword_mult (char *blockname, float mult);

int s_xdr_string (unsigned char *astring);
...
int s_binary_repeat (char *instring, int times);

int s_string_repeat (char *instring, int times);

int s_string_lf (char *instring);
...
unsigned long ioal (unsigned long myint);

/*same for shorts*/
unsigned short ioas (unsigned short myshort);

void intel_order (char *buffer, int length);

/*push a word in intel order*/
int s_intelword (unsigned int aword);

/*pushes a short onto the spike in intel order*/
/*also returns the short in ioas format - who knows.*/
int s_intelhalfword (unsigned short ashort);

/*Dark Side of the Moon*/
int s_binary_block_size_intel_halfword (char *blockname);


int s_binary_block_size_halfword_bigendian (char *blockname);

int s_binary_block_size_intel_halfword_plus (char *blockname, long plus);

int s_binary_block_size_byte (char *blockname);

int s_binary_block_size_byte_plus (char *blockname, long plus);
...
void s_string_or_env (unsigned char *envvar, unsigned char *realvar);

void s_incrementfuzzstring ();

int
s_getfuzzstringindex ();

int s_getfuzzvariableindex();

void s_string_variables (unsigned char splitchar, unsigned char *variables);

void s_string_variable (unsigned char *variable);

void s_resetfuzzstring ();

/*true if we've done the last variable. False otherwise. Survices spike_clear()*/
int s_didlastvariable ();

/*must be called before doing any string fuzzing*/
void s_resetfuzzvariable ();

/*initializes a bunch of strings and throws them into s_fuzzstring,
  which is a global array of strings, ending with a null*/
void s_init_fuzzing ();

int s_didlastfuzzstring ();

void s_incrementfuzzvariable ();

void s_setfirstvariable ();
...
unsigned char *s_scan_for_variable (unsigned char *buffer,
				    unsigned char *startdelim,
				    unsigned char *enddelim);


struct spike *getcurrentspike ();
...
unsigned char *s_get_fuzzstring (unsigned int index);

/*return the maximum fuzz string*/
unsigned int s_get_max_fuzzstring ();

/*gets a random fuzzstring for you*/
/*returns - fuzzstring*/
unsigned char *s_get_random_fuzzstring ();

int s_binary_block_size_intel_word (char *blockname);

int s_binary_block_size_intel_word_plus (char *blockname, long some);

/*pushes an int of type "type"*/
void s_push_int (int value, int type);
...
int s_binary_block_size_byte_variable (char *blockname);
...
void s_unistring (char *instring);
...
int s_read_bigendian_word_packet (char **buf);
