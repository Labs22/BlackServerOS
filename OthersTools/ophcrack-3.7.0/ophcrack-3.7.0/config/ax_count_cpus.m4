##### http://autoconf-archive.cryp.to/ax_count_cpus.html
#
# SYNOPSIS
#
#   AX_COUNT_CPUS
#
# DESCRIPTION
#
#   Attempt to count the number of processors present on the machine.
#   If the detection fails, then a value of 1 is assumed.
#
#   The value is placed in the CPU_COUNT variable.
#
# LAST MODIFICATION
#
#   2007-11-30
#
# COPYLEFT
#
#   Copyright (c) 2007 Michael Paul Bailey <jinxidoru@byu.net>
#   Copyright (c) 2007 Christophe Tournayre <turn3r@users.sourceforge.net>
#
#   Copying and distribution of this file, with or without
#   modification, are permitted in any medium without royalty provided
#   the copyright notice and this notice are preserved.

AC_DEFUN([AX_COUNT_CPUS], [
    AC_REQUIRE([AC_PROG_EGREP])
    AC_MSG_CHECKING(the number of available CPUs)
    MY_CPU_COUNT="0"

    #On MacOS
    if test -x /usr/sbin/sysctl -a `/sbin/sysctl -a 2>/dev/null| grep -c hw.cpu`; then
        MY_CPU_COUNT=`/usr/sbin/sysctl -n hw.ncpu`
    fi

    #On Linux
    if test "x$MY_CPU_COUNT" = "x0" -a -e /proc/cpuinfo; then
        MY_CPU_COUNT=`$EGREP -c '^processor' /proc/cpuinfo`
    fi

    if test "x$MY_CPU_COUNT" = "x0"; then
        MY_CPU_COUNT="1"
        AC_MSG_RESULT( [unable to detect (assuming 1)] )
    else
        AC_MSG_RESULT( $MY_CPU_COUNT )
	AC_DEFINE_UNQUOTED([MY_CPU_COUNT], $MY_CPU_COUNT, [Count of CPUs detected])
    fi
])
