...
sub act
{
    ...
    foreach my $key (keys %{$vector->{data}->{post_params}}) {
        foreach my $exit (@lfi_exits) {
            ...
            $test_vector->{data}->{post_params}->{$key}->{value} = $lfi_test . $exit;
            ...
            if($test_content =~ /$lfi_match/) {
                ...
                $self->api("CORE")->log_vuln($vector, "Discovered local file inclusion (LFI) vulnerability: POST $vector->{data}->{path} -- POST DATA $vector->{vuln}->{key}: included $vector->{vuln}->{lfi_test}$vector->{vuln}->{exit}");
            }
        }
    }
}
