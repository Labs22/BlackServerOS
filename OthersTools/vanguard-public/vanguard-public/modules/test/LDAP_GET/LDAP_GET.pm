...
sub act
{
	...
	foreach my $key (keys %{$vector->{data}->{get_params}}) {
	    ...
	    $true_vector->{data}->{get_params}->{$key}->{value} .= $ldap_true[$i];
	    
	    my $false_vector = $vector->clone();
	    $false_vector->{data}->{get_params}->{$key}->{value} .= $ldap_false[$i];
		...
		$self->api("CORE")->log_vuln($vector, "Discovered LDAP injection vulnerability: $vector->{data}->{method} $vector->{data}->{path} -- URL PARAMETER $vector->{vuln}->{key}");
	    }
	}
	$i++;
    }
